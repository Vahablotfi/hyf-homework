...
function UserStory() {
  ...
  const [employerButton, setEmployerButton] = useState(false);
    ...
    getShifts()
      ...
      const res = await fetch(' https://gist.githubusercontent.com/benna100/5fd674171ea528d7cd1d504e9bb0ca6f/raw')
    ...
    return (
      <div >
          ...
          <button className='popup-btn' onClick={() => setEmployeeButton(true)} >Employee</button>
           <button className='popup-btn' onClick={()=>setEmployerButton(true)} >Employer</button>
        </div>
